...
class DockerMonitor:
    def __init__(self, server_ip):
        ...
        self.persistent_containers = {}  # Persistent tracking of all 
containers
        self.alerted_containers = set()  # Track alerted container IDs

    def get_containers(self):
        """Fetch the list of all containers."""
        url = f"http://{self.server_ip}:2375/containers/json?all=true"
  # Include stopped containers
    ...
    def get_container_stats(self, container_id):
        ...
        url = f"http://{self.server_ip}:2375/containers/{container_id}
/stats?stream=false"
        response = requests.get(url)
        response.raise_for_status()
        return response.json()
    ...
    def calculate_usage(self, stats):
        ...
        prev_total_usage = stats["precpu_stats"]["cpu_usage"]["total_u
sage"]
        system_cpu_usage = stats["cpu_stats"]["system_cpu_usage"]
        prev_system_cpu_usage = stats["precpu_stats"]["system_cpu_usag
e"]
        ...
        cpu_percentage = (cpu_delta / system_delta) * online_cpus * 10
0 if system_delta > 0 else 0

        memory_usage = stats["memory_stats"]["usage"]
        memory_limit = stats["memory_stats"]["limit"]
        memory_percentage = (memory_usage / memory_limit) * 100 if mem
ory_limit > 0 else 0
    ...
    def monitor(self):
        ...
        for container in containers:
            ...
            container_name = container["Names"][0] if container["Names
"] else "Unknown"
        ...
        for container_id, container_data in self.persistent_containers
.items():
            if container_id not in current_containers:  # Stopped cont
ainer
                if container_data["state"] != "stopped":
                    ...
                    if container_id not in self.alerted_containers:
                        events.append(f"Container {container_data['nam
e']} has stopped.")
                        self.alerted_containers.add(container_id)
            elif current_containers[container_id]["state"] == "exited"
:  # Exited container
                if container_data["state"] != "exited":
                    container_data.update({
                        "state": "exited",
                        "status": current_containers[container_id]["st
atus"],
                        "cpu_percentage": "N/A",
                        "memory_percentage": "N/A",
                        "memory_usage_mb": "N/A",
...
class APIHealthMonitor:
    ...
    def check_open_meteo():
        url = OPENMETEO_API
        params = {"latitude": SAMPLE_LAT, "longitude": SAMPLE_LONG, "c
urrent_weather": "true"}
        try:
            response = requests.get(url, params=params)
            return response.status_code == 200
...
class DatabaseHealthMonitor:
    ...
    def monitor(self, mysql_config, influxdb_host, influxdb_port):
        return {
            ...
            "influxdb": self.check_influxdb(influxdb_host, influxdb_po
rt),
        }
